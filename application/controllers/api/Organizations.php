...
class Organizations extends REST_Controller {
    ...
    function index_get() {
        ...
        try {
            if ($id) {        
                $data = $this->db->select('id, parent_id, org_unit, code, level')->from('organizations')->where('id', $id)->get()->result();
            } else {            
                $data = $this->db->select('id, parent_id, org_unit, code, level')->from('organizations')->get()->result();
            }
